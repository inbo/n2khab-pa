...
# Gedeeltelijk mooiere tabellen

```{r results="asis", eval=(uitvoer=="html"), echo=(uitvoer=="html")}
input_rearranged %>% 
    group_by(Vraag) %>% 
    nest(.key = Data) %>% 
    mutate(Richdata = map2(Vraag, Data, 
                          function(vraaglabel, data) {
                              cbind(
                                  tibble(Vraag = as.character(vraaglabel)),
                                  data
                              )
                          }
    )
    ...
    lapply(function(df) {
        ...
        tablecap <- str_c("Vraag ", unique(df$Vraag %>% as.character))
