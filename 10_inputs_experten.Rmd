...
input_rearranged <- 
    ...
    group_by(Vraag) %>% 
    nest(.key = Data) %>% 
    mutate(output = map2(Vraag, Data, 
                       function(vraaglabel, data) {
                           
                           knitr::knit_expand("10b_child_doc.Rmd") %>% 
                               unlist
                       
                        }
                       ))

output.knit <- knitr::knit_child(text = unlist(input_rearranged$output), quiet = TRUE)
...
<!-- `r paste(knit(text = unlist(input_rearranged$output)), collapse = '\n')` -->
